...
import { Trophy, Users, Calendar, Goal, Zap, Loader2, X, ArrowUpRight, ArrowDownLeft } from "lucide-react";
...
export default function DashboardPage() {
  const { 
    teams, players, matches, isLoaded, divisions, 
    refreshData, getMatchEvents, getTeamById, lastPlayedWeek 
  ...
  const handleSimulateAll = async () => {
    ...
    try {
      ...
      for (const divId of divisionIds) {
        // Usamos lastPlayedWeek + 1 para simular la siguiente jornada disponible
        const nextWeek = matches.some(m => !m.played) ? lastPlayedWeek : lastPlayedWeek + 1;
        
        const res = await fetch("/api/match/simulate-matchday", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ divisionId: divId, week: nextWeek }),
        });
        if (!res.ok) throw new Error(`Error en división ${divId}`);
      }
      toast.success("¡Jornada simulada exitosamente!");
                      ...
                      >
                        <div className={`flex items-center gap-4 max-w-[48%] ${isHomeEvent ? 'flex-row' : 'flex-row-reverse text-right'}`}>
                          ...
                          <div className="shrink-0 z-10">
                            {event.type === 'GOAL' && (
                              <div className="bg-slate-100 w-8 h-8 rounded-full flex items-center justify-center shadow-sm border border-slate-200 text-sm">⚽</div>
                            )}
                            ...
                            {/* LÓGICA DE ICONO PARA SUSTITUCIÓN */}
                          ...
                          <div className="flex flex-col overflow-hidden">
                            {/* Ajuste para mostrar Entrada/Salida en Cambios */}
                            {event.type === 'SUBSTITUTION' ? (
                              <>
                                <span className="text-xs font-black text-green-600 truncate tracking-tight">
                                  {event.player_name || event.playerName}
                                </span>
                                <span className="text-[10px] text-red-400 font-bold truncate">
                                  {event.player_out_name || event.playerOutName}
                                </span>
                              </>
                            ) : (
                              <>
