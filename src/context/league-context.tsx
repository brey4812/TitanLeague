...
export const LeagueProvider = ({ children }: { children: ReactNode }) => {
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      const [matchesRes, eventsRes, seasonRes] = await Promise.all([
        ...
        // CORRECCIÓN 1: Nombre de columna exacto 'season_number'
  ...
  const autoMatchmaker = useCallback(async () => {
    ...
    for (const div of divisions) {
      ...
      if (targetWeek > (n - 1) * 2 || divMatches.some(m => Number(m.round) === targetWeek)) continue;
  ...
  const processedTeams = useMemo(() => {
    // CORRECCIÓN 2: Protección de Hidratación (Evita Error #310)
  ...
  const getBestEleven = useCallback((type: string, value?: number): TeamOfTheWeekPlayer[] => {
    ...
    const activeS = currentSeason;
    if (type === 'week' && value) {
      filteredMatchIds = matches.filter(m => Number(m.round) === value && (Number(m.season) || 1) === activeS && m.played).map(m => String(m.id));
    } else {
      filteredMatchIds = matches.filter(m => (Number(m.season) || 1) === activeS && m.played).map(m => String(m.id));
    }
  ...
  const lastPlayedWeek = useMemo(() => {
    ...
    return playedMatches.length === 0 ? 1 : Math.max(...playedMatches.map(m => Number(m.round || 0)));
  }, [matches, currentSeason]);

  // --- CORRECCIÓN VALUE PARA TYPES.TS ---
  const value: LeagueContextType = {
    ...
    getLeagueQualifiers: (divisionId) => {
      const divTeams = processedTeams.filter(t => Number(t.division_id) === divisionId).sort((a,b) => (b.points || 0) - (a.points || 0));
    ...
    drawTournament: async (name) => console.log(name),
    resetLeagueData: async () => { if(confirm("¿Reset?")) { await supabase.from('matches').delete().eq('session_id', sessionId); window.location.reload(); } },
    importLeagueData: (d) => true,
    refreshData
  };
